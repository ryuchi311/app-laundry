...
def dashboard():
    ...
    from .models import Customer, CustomerLoyalty, LoyaltyProgram, LoyaltyTransaction
    ...
    stats = {
        ...
        "total_points_earned": db.session.query(
            db.func.sum(CustomerLoyalty.total_points_earned)
        ).scalar()
        or 0,
        "total_points_redeemed": db.session.query(
            db.func.sum(CustomerLoyalty.total_points_redeemed)
        ).scalar()
        or 0,
        "active_members": CustomerLoyalty.query.filter(
            CustomerLoyalty.current_points > 0
        ).count(),
    }

    # Get recent transactions
    recent_transactions = (
        LoyaltyTransaction.query.order_by(LoyaltyTransaction.created_at.desc())
        .limit(5)
        .all()
    )
...
def customer_detail(customer_id):
    ...
    page = request.args.get("page", 1, type=int)
    transactions = (
        LoyaltyTransaction.query.join(
            CustomerLoyalty,
            LoyaltyTransaction.customer_loyalty_id == CustomerLoyalty.id,
        )
    ...
    return render_template(
        "loyalty/customer_detail.html", customer=customer, transactions=transactions
    )
...
def update_settings():
    ...
    try:
        ...
        program.peso_per_point = (
            (1.0 / ppd_val) if ppd_val and ppd_val > 0 else program.peso_per_point
        )
        ...
        if tier_thresholds:
            try:
                parts = [
                    int(x.strip())
                    for x in tier_thresholds.split(",")
                    if x.strip() != ""
                ]
                # Map parts to model fields if provided
                if len(parts) >= 1:
                    program.bronze_threshold = parts[0]
                if len(parts) >= 2:
                    program.silver_threshold = parts[1]
                if len(parts) >= 3:
                    program.gold_threshold = parts[2]
                if len(parts) >= 4:
                    program.platinum_threshold = parts[3]
            except Exception:
                pass
...
def award_points():
    ...
    try:
        ...
        db.session.commit()
        flash(
            f"Successfully awarded {points} points to {customer.full_name}!",
            category="success",
        )
...
def redeem_points():
    ...
    try:
        ...
        db.session.commit()
        flash(
            f"Successfully redeemed {points} points from {customer.full_name}!",
            category="success",
        )
...
def bulk_award_points():
    ...
    try:
        ...
        db.session.commit()
        flash(
            f"Successfully awarded {points} points to {customers_awarded} customers! Total points awarded: {total_points_awarded}",
            category="success",
        )
