...
class User(db.Model, UserMixin):
    ...
    def is_user(self):
        """Check if user is basic user (basic access)"""
    ...
    def can_manage_customers(self):
        ...
        return self.role in ["user", "manager", "admin", "super_admin"]
...
class Customer(db.Model):
    ...
    def is_regular_customer(self):
        ...
        return loyalty.total_orders >= 5 or loyalty.current_tier in [
            "Silver",
            "Gold",
            "Platinum",
        ]
            ...
            try:
                ...
                if service:
                    return service.calculate_total_price(
                        self.item_count, self.weight_kg
                    )
            except Exception:
                # Fallback below
                pass
...
class InventoryCategory(db.Model):
    ...
    items = db.relationship(
        "InventoryItem", backref="category", lazy=True, cascade="all, delete-orphan"
    )
...
class InventoryItem(db.Model):
    ...
    category_id = db.Column(
        db.Integer, db.ForeignKey("inventory_category.id"), nullable=False
    )
    ...
    unit_of_measure = db.Column(
        db.String(20), default="pieces"
    )  # pieces, liters, kg, bottles, etc.
    ...
    # Status
    is_active = db.Column(db.Boolean, default=True)
    is_consumable = db.Column(
        db.Boolean, default=True
    )  # True for supplies, False for equipment
    ...
    stock_movements = db.relationship(
        "StockMovement", backref="item", lazy=True, cascade="all, delete-orphan"
    )
...
class Expense(db.Model):
    ...
    expense_type = db.Column(
        db.String(20), default="ONE_TIME"
    )  # ONE_TIME, RECURRING, MAINTENANCE
...
class LoyaltyProgram(db.Model):
    ...
    peso_per_point = db.Column(
        db.Float, default=0.10
    )  # Peso value per point when redeeming
    min_spend_for_points = db.Column(
        db.Float, default=50.0
    )  # Minimum spend to earn points
    min_points_to_redeem = db.Column(
...
class CustomerLoyalty(db.Model):
    ...
    customer_id = db.Column(
        db.Integer, db.ForeignKey("customer.id"), nullable=False, unique=True
    )
    ...
    point_transactions = db.relationship(
        "LoyaltyTransaction", backref="customer_loyalty", lazy=True
    )

    def update_tier(self, loyalty_program):
        """Update customer tier based on total points earned"""
        tier, multiplier, icon, color = loyalty_program.get_tier_info(
    ...
    def can_redeem(self, points_to_redeem, loyalty_program):
        ...
        return (
            ...
            and points_to_redeem >= loyalty_program.min_points_to_redeem
        )
...
class LoyaltyTransaction(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    customer_loyalty_id = db.Column(
        db.Integer, db.ForeignKey("customer_loyalty.id"), nullable=False
    )
    ...
    # Related Records
    laundry_id = db.Column(
        db.Integer, db.ForeignKey("laundry.id"), nullable=True
    )  # If earned from order
    order_amount = db.Column(
        db.Float, nullable=True
    )  # Order amount that generated points
...
class SMSSettings(db.Model):
    ...
    ready_pickup_message = db.Column(
        db.Text,
        default="Hi {customer_name}! Great news! Your laundry (#{laundry_id}) is ready for pickup. Please visit us during business hours. - {sender_name}",
    )
    completed_message = db.Column(
        db.Text,
        default="Hi {customer_name}! Your laundry (#{laundry_id}) has been completed. Thank you for choosing {sender_name}!",
    )
    welcome_message = db.Column(
        db.Text,
        default="Welcome to {sender_name}, {customer_name}! We're excited to serve you. For inquiries, contact us at +639761111464.",
    )
...
class SMSSettingsProfile(db.Model):
    ...
    def create_from_active(
        name: str, user_id: int | None = None, make_default: bool = False
    ):
...
class ExportAudit(db.Model):
    ...
    def __repr__(self):
        return f"<ExportAudit user={self.user_id} search={self.search_query} at={self.created_at}>"
...
class BusinessSettings(db.Model):
    ...
    business_description = db.Column(
        db.Text, default="Professional laundry services with quality care"
    )
    ...
    address = db.Column(
        db.Text,
        default="Purok 17, Lower Mandacpan, Brgy. San Vicente, Butuan City, Philippines",
    )
    ...
    copyright_text = db.Column(
        db.String(200), default="© 2025 ACCIO Labhonon Laundry. All rights reserved."
    )
