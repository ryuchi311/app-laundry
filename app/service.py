...
from . import db
from .models import Laundry, Service
...
def list_services():
    ...
    active_services = Service.query.filter(Service.is_active).count()
...
def add_service():
    if request.method == "POST":
        ...
        if (
            not estimated_hours
            or not estimated_hours.isdigit()
            or int(estimated_hours) <= 0
        ):
...
def delete_service(id):
    ...
    if laundries_using_service > 0:
        flash(
            f'Cannot delete service "{service_obj.name}" because it is being used in {laundries_using_service} laundry item(s). Please deactivate instead.',
            category="error",
        )
    else:
