...
FIRST_NAMES = [
    ...
    "Ulysses", "Vera", "Warren", "Xandra", "Yuri", "Zara"
]
...
MIDDLE_NAMES = [
    ...
    "Ruiz", "Salazar", "Vergara", "Villanueva", "Dela Cruz", "Del Rosario", "Rosales", "Sarmiento"
]

LAST_NAMES = [
    "Santos", "Reyes", "Cruz", "Bautista", "Garcia", "Lopez", "Gonzales", "Torres", "Ramos", "Aquino",
    "Navarro", "Mendoza", "Castro", "Perez", "Flores", "De Leon", "Gutierrez", "Domingo", "Valdez", "Padilla",
    "Alvarez", "Bernardo", "Clemente", "Esparza", "Ferrer", "Herrera", "Ibanez", "Jimenez", "Laguna", "Manalo",
    "Natividad", "Ocampo", "Panganiban", "Querubin", "Rivera", "Solis", "Trias", "Umali", "Velasco", "Yabut"
]
...
def gen_name_email(existing_emails: set[str], existing_names: set[str], batch_emails: set[str], batch_names: set[str], used_firstnames: set[str]) -> tuple[str, str]:
    ...
    for _ in range(30):
        ...
        if base_first in used_firstnames:
            ...
            if base_first in used_firstnames:
                ...
                base_first = base_first + ''.join(random.choices(string.ascii_uppercase + string.digits, k=2))
        name_parts[0] = base_first

        full_name = f"{name_parts[0]} {' '.join(name_parts[1:])} {ln}".replace("  ", " ").strip()
    ...
    if (full_name not in existing_names and full_name not in batch_names and
        email not in existing_emails and email not in batch_emails and
        _first_token(full_name) not in used_firstnames):
...
def add_customers(count: int, commit_every: int = 100) -> int:
    ...
    for _ in range(count):
        name, email = gen_name_email(existing_emails, existing_names, batch_emails, batch_names, existing_firstnames | batch_firstnames)
        c = Customer()
        c.full_name = name
        c.email = email
        c.phone = rand_phone()
...
def main():
    ...
    parser.add_argument("--count", type=int, default=200, help="How many customers to create (default: 200)")
